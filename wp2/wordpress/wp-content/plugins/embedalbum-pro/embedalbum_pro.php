...
class EmbedSocialPlugin {
    ...
    public function embedsocial_feed_shortcode( $atts ) 
    {
        ...
        if ($shortcodeId['id']) {
            $shortcodeId['id'] = sanitize_text_field($shortcodeId['id']);
            $out .= "<script src='https://embedsocial.com/js/iframe.js'></script><iframe style='border: 0; width: 100%; height: 100%;' scrolling=no src='https://embedsocial.com/facebook_album/pro_embed_posts/{$shortcodeId['id']}'></iframe><script>iFrameResize();</script>";
        }
        return $out;
    }
